...
return function (\shgysk8zer0\DOM\HTML $dom, \shgysk8zer0\Core\PDO $pdo)
{
	...
	$publisher = $pdo->nameValue('publisher');
	if (isset($publisher, $publisher->name)) {
		...
		$container->append('meta', null, [
			'content' => isset($publisher->url) ? $publisher->url : \KVSun\DOMAIN,
			'itemprop' => 'url',
		]);
		if (isset($publisher->logo)) {
			...
			$logo->append('meta', null, [
				'content' => \KVSun\DOMAIN . ltrim($publisher->logo),
				'itemprop' => 'url',
			]);
		}
		...
		if (isset(
			...
			if (isset($publisher->POBox)) {
				$address->append('div', 'PO Box&nbsp;')->append('span', $publisher->POBox, [
