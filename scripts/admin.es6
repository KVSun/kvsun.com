...
function buildArticleForm(name) {
	...
	details.appendChild(document.createElement('br'));
	details.appendChild(sortLabel);
	details.appendChild(sort);
	details.appendChild(document.createElement('br'));
	...
	sort.max = 9;
	sort.required = true;
	sort.name = `${form.name}[sort]`;
	sort.id = `${form.name}-sort`;
	sortLabel.textContent = 'Priority';
	sortLabel.setAttribute('for', sort.id);
